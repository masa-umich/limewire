...
class IP_Address_UI:
    def __init__(self, initial : ipaddress.IPv4Address, name : str):
        ...
        with ui.row().classes("no-wrap items-end gap-1"):
            ...
            oct0 = ui.input(value=self.get_octet(0), on_change=IP_UI_Octet_Handle(self, 0)).props("dense outlined").classes("min-w-[4em] w-[4em]")
            ui.label(".")
            oct1 = ui.input(value=self.get_octet(1), on_change=IP_UI_Octet_Handle(self, 1)).props("dense outlined").classes("min-w-[4em] w-[4em]")
            ui.label(".")
            oct2 = ui.input(value=self.get_octet(2), on_change=IP_UI_Octet_Handle(self, 2)).props("dense outlined").classes("min-w-[4em] w-[4em]")
            ui.label(".")
            oct3 = ui.input(value=self.get_octet(3), on_change=IP_UI_Octet_Handle(self, 3)).props("dense outlined").classes("min-w-[4em] w-[4em]")
    ...
    def __call__(self, e):
        ...
        if(new_val == ""):
            ...
            if(int(new_val) < 256 and int(new_val) >= 0):
                ...
                ui.timer(0, lambda: e.sender.set_value(self.parent.get_octet(self.oct)), active=True, once=True)
                return
        
        ui.timer(0, lambda: e.sender.set_value(e.previous_value), active=True, once=True)
...
class PT_UI:
    def __init__(self, range : float, offset : float, max : float, name : str):
        ...
        with ui.column().classes("w-full gap-1"):
            ...
            ui.separator()
            ui.number(label="Range", value=range).props("filled dense").bind_value(self, "range")
            ui.number(label="Offset", value=offset).props("filled dense").bind_value(self, "offset")
            ui.number(label="Max", value=max).props("filled dense").bind_value(self, "max")
...
class TC_UI:
    def __init__(self, gain : int, name : str):
        ...
        with ui.row().classes("w-full mx-auto no-wrap gap-1 items-center"):
            ...
            ui.select([1, 2, 4, 8, 16, 32, 64, 128], value=gain, label="Gain").props("filled dense").classes("min-w-25").bind_value(self, "gain")
...
class Valve_UI:
    def __init__(self, enabled : bool, voltage : int, name : str):
        ...
        with ui.column().classes("w-full gap-1"):
            ...
            ui.select([12, 24], value=voltage, label="Voltage").props("filled dense").classes("min-w-25").bind_value(self, "voltage")
                ...
                with ui.column().classes("border-1 p-2 border-gray-500 flex-1 basis-auto"):
                    with ui.row().classes("w-full mx-auto no-wrap gap-3"):
                        for x in range(NUM_FC_PTs):
                            self.PTs.append(PT_UI(DEFAULT_PT_RANGE, DEFAULT_PT_OFFSET, DEFAULT_PT_MAX, "PT " + str(x + 1)))
                # TCs
                with ui.column().classes("border-1 p-2 border-gray-500 gap-3 flex-1 basis-auto justify-center"):
                    with ui.column().classes("w-full gap-3"):
                        for x in range(NUM_FC_TCs):
                            self.TCs.append(TC_UI(DEFAULT_TC_GAIN, "TC " + str(x + 1) + " "))
                # Valves
                with ui.column().classes("border-1 p-2 border-gray-500 flex-1 basis-auto"):
                    with ui.row().classes("w-full mx-auto no-wrap gap-3"):
                        for x in range(NUM_FC_VLVs):
                            self.valves.append(Valve_UI(DEFAULT_VALVE_ENABLED, DEFAULT_VALVE_VOLTAGE, "Valve " + str(x + 1)))
            with ui.row().classes("w-full mx-auto no-wrap flex items-stretch h-full border-1 p-2 border-gray-500"):
                ...
                with ui.column().classes("w-full"):
                    with ui.column().classes("w-fit items-end"):
                        self.limewireIP = IP_Address_UI(DEFAULT_GSE_IP, "Limewire IP")
                        self.FCIP = IP_Address_UI(DEFAULT_FC_IP, "Flight Computer IP")
                with ui.column().classes("w-full"):
                    with ui.column().classes("w-fit items-end"):
                        self.BB1IP = IP_Address_UI(DEFAULT_BB1_IP, "Bay Board 1 IP")
                        self.BB2IP = IP_Address_UI(DEFAULT_BB2_IP, "Bay Board 2 IP")
                with ui.column().classes("w-full"):
                    with ui.column().classes("w-fit items-end"):
                        self.BB3IP = IP_Address_UI(DEFAULT_BB3_IP, "Bay Board 3 IP")
...
class System_Config_UI:
        ...
        with ui.card().classes("w-full bg-gray-900 border border-gray-700 p-6"):
            with ui.row().classes("w-full mx-auto no-wrap"):
                with ui.column().classes():
                    ...
                    self.ICD_file = ui.upload(label="Load from ICD", on_upload=self.handle_ICD).props('accept=.xlsx no-thumbnails no-icon auto__false color=lime text-color=black')
                    self.config_button = ui.button("Write Configuration", color="orange", on_click=self.warn_write_config).classes("text-base w-full")
                with ui.column().classes("gap-0 pl-10"):
                    self.all_select = ui.checkbox().classes("h-11").on("click", self.handle_all_check)
                    self.ebox_select = ui.checkbox("EBox").classes("h-8").on("click", self.handle_device_select).bind_value(self, "configure_ebox").bind_enabled_from(self, "ICD_config", backward=self.handle_ebox_select)
                    self.fc_select = ui.checkbox("Flight Computer").classes("h-8").on("click", self.handle_device_select).bind_value(self, "configure_fc")
                    self.bb1_select = ui.checkbox("Bay Board 1 (Press)").classes("h-8").on("click", self.handle_device_select).bind_value(self, "configure_bb1")
                    self.bb2_select = ui.checkbox("Bay Board 2 (Intertank)").classes("h-8").on("click", self.handle_device_select).bind_value(self, "configure_bb2")
                    self.bb3_select = ui.checkbox("Bay Board 3 (Engine)").classes("h-8").on("click", self.handle_device_select).bind_value(self, "configure_bb3")
                    self.fr_select = ui.checkbox("Flight Recorder").classes("h-8").on("click", self.handle_device_select).bind_value(self, "configure_fr").props("disable")
            ...
            with ui.row().classes("w-full mx-auto no-wrap gap-0 justify-between"):
                with ui.column().classes('items-center gap-0') as ebox_prog:
                    with ui.tooltip() as ebox_tooltip:
                        self.ebox_prog_tooltip = ui.html(sanitize=False).classes("text-[12px]").style('''display:inline-block;width:max-content;white-space:normal;max-width:100em;''')
                        self.ebox_prog_tooltip.set_visibility(False)
                        ebox_tooltip.bind_visibility_from(self.ebox_prog_tooltip, "visible")
                    self.ebox_progress_tool = ebox_prog
                    ui.label('EBox').classes('text-sm')
                    with ui.element().classes("relative w-8 h-8 mt-2"):
                        self.ebox_progress = ui.icon("", size="2em").classes("absolute inset-0 flex m-auto").bind_visibility_from(self, "ebox_loading", backward=lambda v: not v)
                        ui.spinner(color="white", size="2em").classes("absolute inset-0 m-auto").bind_visibility_from(self, "ebox_loading")
                with ui.column().classes('items-center gap-0') as fc_prog:
                    with ui.tooltip() as fc_tooltip:
                        self.fc_prog_tooltip = ui.html(sanitize=False).classes("text-[12px]").style('''display:inline-block;width:max-content;white-space:normal;max-width:100em;''')
                        self.fc_prog_tooltip.set_visibility(False)
                        fc_tooltip.bind_visibility_from(self.fc_prog_tooltip, "visible")
                    self.fc_progress_tool = fc_prog
                    ui.label('Flight Computer').classes('text-sm')
                    with ui.element().classes("relative w-8 h-8 mt-2"):
                        self.fc_progress = ui.icon("", size="2em").classes("absolute inset-0 flex m-auto").bind_visibility_from(self, "fc_loading", backward=lambda v: not v)
                        ui.spinner(color="white", size="2em").classes("absolute inset-0 m-auto").bind_visibility_from(self, "fc_loading")
                with ui.column().classes('items-center gap-0') as bb1_prog:
                    with ui.tooltip() as bb1_tooltip:
                        self.bb1_prog_tooltip = ui.html(sanitize=False).classes("text-[12px]").style('''display:inline-block;width:max-content;white-space:normal;max-width:100em;''')
                        self.bb1_prog_tooltip.set_visibility(False)
                        bb1_tooltip.bind_visibility_from(self.bb1_prog_tooltip, "visible")
                    self.bb1_progress_tool = bb1_prog
                    ui.label('Bay Board 1').classes('text-sm')
                    with ui.element().classes("relative w-8 h-8 mt-2"):
                        self.bb1_progress = ui.icon("", size="2em").classes("absolute inset-0 flex m-auto").bind_visibility_from(self, "bb1_loading", backward=lambda v: not v)
                        ui.spinner(color="white", size="2em").classes("absolute inset-0 m-auto").bind_visibility_from(self, "bb1_loading")
                with ui.column().classes('items-center gap-0') as bb2_prog:
                    with ui.tooltip() as bb2_tooltip:
                        self.bb2_prog_tooltip = ui.html(sanitize=False).classes("text-[12px]").style('''display:inline-block;width:max-content;white-space:normal;max-width:100em;''')
                        self.bb2_prog_tooltip.set_visibility(False)
                        bb2_tooltip.bind_visibility_from(self.bb2_prog_tooltip, "visible")
                    self.bb2_progress_tool = bb2_prog
                    ui.label('Bay Board 2').classes('text-sm')
                    with ui.element().classes("relative w-8 h-8 mt-2"):
                        self.bb2_progress = ui.icon("", size="2em").classes("absolute inset-0 flex m-auto").bind_visibility_from(self, "bb2_loading", backward=lambda v: not v)
                        ui.spinner(color="white", size="2em").classes("absolute inset-0 m-auto").bind_visibility_from(self, "bb2_loading")
                with ui.column().classes('items-center gap-0') as bb3_prog:
                    with ui.tooltip() as bb3_tooltip:
                        self.bb3_prog_tooltip = ui.html(sanitize=False).classes("text-[12px]").style('''display:inline-block;width:max-content;white-space:normal;max-width:100em;''')
                        self.bb3_prog_tooltip.set_visibility(False)
                        bb3_tooltip.bind_visibility_from(self.bb3_prog_tooltip, "visible")
                    self.bb3_progress_tool = bb3_prog
                    ui.label('Bay Board 3').classes('text-sm')
                    with ui.element().classes("relative w-8 h-8 mt-2"):
                        self.bb3_progress = ui.icon("", size="2em").classes("absolute inset-0 flex m-auto").bind_visibility_from(self, "bb3_loading", backward=lambda v: not v)
                        ui.spinner(color="white", size="2em").classes("absolute inset-0 m-auto").bind_visibility_from(self, "bb3_loading")
                with ui.column().classes('items-center gap-0') as fr_prog:
                    with ui.tooltip() as fr_tooltip:
                        self.fr_prog_tooltip = ui.html(sanitize=False).classes("text-[12px]").style('''display:inline-block;width:max-content;white-space:normal;max-width:100em;''')
                        self.fr_prog_tooltip.set_visibility(False)
                        fr_tooltip.bind_visibility_from(self.fr_prog_tooltip, "visible")
                    self.fr_progress_tool = fr_prog
                    ui.label('Flight Recorder').classes('text-sm')
                    with ui.element().classes("relative w-8 h-8 mt-2"):
                        self.fr_progress = ui.icon("", size="2em").classes("absolute inset-0 flex m-auto").bind_visibility_from(self, "fr_loading", backward=lambda v: not v)
                        ui.spinner(color="white", size="2em").classes("absolute inset-0 m-auto").bind_visibility_from(self, "fr_loading")
    ...
    def handle_device_select(self, e):
        if(self.ebox_select.value == True 
            and self.fc_select.value == True 
            and self.bb1_select.value == True
            and self.bb2_select.value == True
            and self.bb3_select.value == True
            and self.fr_select.value == True):
    ...
    async def handle_ICD(self, e):
        ...
        try:
            ...
            with ui.dialog().props('persistent') as dialog, ui.card().classes("bg-[#990000] w-250 h-60 flex flex-col justify-center items-center"):
