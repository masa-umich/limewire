...
def set_up_logging(debug: bool, print_dir=False):
    ...
    if print_dir:
        ...
        )  # This should stay as a print statement, as it is just a message to aid in finding the log directory
    logger.level("EVENT", no=1, color="<white>")
    logger.add(
        event_file_path,
        format=log_format,
        level="EVENT",
        filter=lambda r: r["level"].name == "EVENT",
        enqueue=True,
    )
