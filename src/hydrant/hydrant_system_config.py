...
class ICD:
    def __init__(self, content : bytes, name: str):
        self.name = name
        ICD_df = pd.read_excel(BytesIO(content), header=1, sheet_name=ICD_SHEET)
        ICD_df["Type"] = ICD_df["Type"].ffill(axis=0)
        ICD_df["Connection Location"] = ICD_df["Connection Location"].ffill(axis=0)
        self.ebox_channels = []
        self.fc_channels = []
        self.bb1_channels = []
        self.bb2_channels = []
...
def configure_ebox(channels) -> tuple[bool, str]:
    try:
        synnax_client = synnax_login("127.0.0.1") # this should be running on the same machine as synnax
        analog_task, digital_task, analog_card = create_tasks(synnax_client, 50)
        setup_channels(synnax_client, channels, analog_task, digital_task, analog_card)
    ...
    analog_task.config.channels.append(
        ni.AIVoltageChan(
            ...
            custom_scale=ni.LinScale(
                slope=(
                    channel["max"] / (channel["max_voltage"] - channel["offset"]) # slope is max output in psi over output range (4 volts for our PTs)
                ),
                y_intercept=(
                    -(channel["max"] / (channel["max_voltage"] - channel["offset"])) * channel["offset"] # y intercept is negative slope at 0.5 volts
                ),
...
def synnax_login(cluster: str) -> sy.Synnax:
    try:
        ...
        raise Exception(f"Could not connect to Synnax at {cluster}, check to make sure Synnax is running. Original error: {str(e)}")
    return client
