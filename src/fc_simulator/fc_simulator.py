...
class FCSimulator:
    ...
    async def handle_client(
        ...
        valve_task = asyncio.create_task(
            self.handle_valve_commands(reader, writer)
        )
    ...
    async def run(self, ip_addr: str, port: int, run_time: float) -> None:
        ...
        server = await asyncio.start_server(
            partial(self.handle_client, run_time=run_time),
            ip_addr,
            port,
        )
